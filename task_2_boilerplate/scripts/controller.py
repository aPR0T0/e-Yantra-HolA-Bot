...
def inverse_kinematics():
	...
	if(len(x_goals) != 0 or len(y_goals) != 0 or len(theta_goals) != 0):
		V_x = x_goals[index] 
		V_y = y_goals[index]
		theta = theta_goals[index]
		constants_matrix = np.array([[1,-0.5,math.sqrt(3)/2],[0, -0.5, 0.5],[-1,-1,-1]])
		given_goal_matrix = np.array([V_x, V_y, theta])
		...
		print("Error in inverse_kinematics , one or more goals empty, problem in topic subscription")
