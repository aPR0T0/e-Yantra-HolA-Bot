...
const float kp_x = 0.04;
const float kp_y = 0.04;
const float kp_z = 0.04;
...
// Task 5B : Mode1
const int N = 1e3; // Maximum 1000 Way points, nad maximum number of countours is 10
double goals_countours[N][3];
...
void stepper_task(void *arg){
  ...
  printf("Please enter the mode you want \n");
  printf("Mode 0 : Function Mode\n");
  printf("Mode 1 : Countour Mode\n");

  int Mode = 0 ;
  scanf("Enter Here : %d", &Mode, "\n");

  enable_servo();

	while(1){

    current_x     = read_pid_const().kp;  // This is a constant for every mode
    current_y     = read_pid_const().ki;  // This is a constant for every mode
    current_theta = read_pid_const().kd;  // This is a constant for every mode

    curr_time = timer();
    curr_time_seconds =  curr_time / 1000000; // As time was in microseconds

    if(Mode == 0){  // Function Mode
      printf("Using User Entered Function\n");
      /*
       * Some Function that will take the value of the function in terms of t from the user
      */
      // Default Function
      goal_x = 400*cos(curr_time);
      goal_y = 200*sin(2*curr_time);
      goal_theta = (M_PI / 4)*sin(curr_time);

      
    }
    else if(Mode == 1){
      printf("Using user given waypoints from a countour\n");
      
      goal_x = goals_countours[idx][0];   
      goal_y = goals_countours[idx][1];
      goal_theta = 0;                               // Will hold its orientation

      if(goal_x = -1){
        set_angle_servo(&servo_a, 0);   // Setting to the zero position but needs to be tweaked for the use
        set_angle_servo(&servo_b, 0);

        goal_x = current_x; // doing this change so that bot doesn't move when the goals are set to -1
        goal_y = current_y; // doing this change so that bot doesn't move when the goals are set to -1
