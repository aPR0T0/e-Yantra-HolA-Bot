...
void speed_publisher(double x1, double x2, double x3, double vel_1, double vel_2, double vel_3){	// Publishes Speed according to the given body frame velocities
  ...
  vel_1 = abs(vel_1);   // Direction pins are already set 
  vel_2 = abs(vel_2);   // Direction pins are already set 
  vel_3 = abs(vel_3);   // Direction pins are already set 
  
  printf("print %f",vel_2);
...
void stepper_task(void *arg){
  ...
  // put your main code here, to run repeatedly:
	...
	float err_x = 0, err_y = 0, err_theta = 0;
  ...
  m3_timer_prev = timer();
    ...
    vel_x = kp_x*err_x;  // v = r*Ω*sin(Θ) 
    vel_y = kp_y*err_y;  // v = r*Ω*cos(Θ)
