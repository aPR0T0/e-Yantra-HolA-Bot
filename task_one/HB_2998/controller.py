...
def task1_goals_Cb(msg):
	...
	if(len(x_goals) == 0 or len(y_goals) == 0 or len(theta_goals) == 0):
		...
		# des_x, des_y, des_theta =  x_goals[0], y_goals[0], theta_goals[0]
...
def main():
		global pub, vel_x, vel_y,vel_z, kp_x, kp_y, kp_theta, rotation_matrix, current_time, Helper_time, index, des_x, des_y, des_theta, flag
		# Initialze Node
		...
		# err_x_pub = rospy.Publisher("/err_x", Float64, queue_size=100)
		
		# err_y_pub = rospy.Publisher("/err_y", Float64, queue_size=100)
		
		# err_theta_pub = rospy.Publisher("/err_theta", Float64, queue_size=100)
