...
def inverse_kinematics(vel_x, vel_y, vel_z):
	...
	############################################
	constants_matrix = np.array([[1,-0.5,-0.5],[0, -math.sqrt(3)/2, math.sqrt(3)/2],[-1,-1,-1]])
	given_goal_matrix = np.array([[float(vel_x)], [float(vel_y)], [float(vel_z)]])
	# print(given_goal_matrix)
...
def main():
	...
	while not rospy.is_shutdown():
		...
		if (des_x - 5 <= hola_x <= 5 + des_x and des_y - 5 <= hola_y <= des_y + 5 and  des_theta - 2 <= hola_theta <= des_theta + 2):
